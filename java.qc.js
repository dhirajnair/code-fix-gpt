...
async function qcIssue(filePath, issue) {
    try {
        ...
        const systemContent = "Context: You are Ack, a detail-oriented software developer, with a strong background in Java and related technology stack, specifically Java 8, Tomcat 8.5, Junit, Struts 2.3, EJB 3, Hibernate, Spring Framework, Oracle database, VoltDB, XML, Security vulnerability.\n\
                              ...
                              - If the issue exists then qc_fix will be false else it will be true.If qc_fix is false provide the reason for the same in qc_fix_reason.\n\
                              - if there is a syntax issue in code then qc_syntax will be false else it will be true.If qc_syntax is false provide the reason for the same in qc_syntax_reason.\n\
                              -Sample JSON below:\n\
